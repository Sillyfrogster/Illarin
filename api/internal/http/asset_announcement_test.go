...
func TestAPublishedUpdateAnnouncesToItsChosenDestinationsOutsideTheRequest(t *testing.T) {
	...
	webhookID := arrivals[0].Headers.Get(webhook.IDHeader)
	stamp := stampOf(t, arrivals[0])
	if !webhook.Accepts(arrivals[0].Headers.Get(webhook.SignatureHeader), hook.Secret, webhookID, stamp, arrivals[0].Body) {
...
func TestAnnouncementsRetryOnTheSharedScheduleWithAnInjectedClock(t *testing.T) {
	...
	at := time.Now().UTC()
	for place, gap := range outbox.Delays[1:] {
	...
	if spent.State != "failed" || spent.SettledReason != "exhausted" || spent.Attempts != outbox.Attempts {
		t.Errorf("the run ended %+v, want failed/exhausted after %d attempts", spent, outbox.Attempts)
	}
	...
	for _, one := range stack.to.arrivals() {
		if one.Headers.Get(webhook.IDHeader) != spent.ID {
...
func TestARotatedSecretSignsAnnouncementsTwiceDuringTheOverlap(t *testing.T) {
	...
	id, stamp := arrivals[0].Headers.Get(webhook.IDHeader), stampOf(t, arrivals[0])
	signature := arrivals[0].Headers.Get(webhook.SignatureHeader)
	if !webhook.Accepts(signature, hook.Secret, id, stamp, arrivals[0].Body) ||
		!webhook.Accepts(signature, fresh.Secret, id, stamp, arrivals[0].Body) {
