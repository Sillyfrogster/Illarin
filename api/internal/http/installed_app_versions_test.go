...
func linkInstallations(t *testing.T, r *gin.Engine, session *http.Cookie, count int) []tokenGrant {
	t.Helper()
	grants := make([]tokenGrant, 0, count)
	for index := range count {
		grant := linkDeviceInstance(
			t, r, session, "Lumiverse", fmt.Sprintf("desk %d", index+1),
			[]string{receiveScope, librarySyncScope},
		)
...
func TestAnExtensionPageListsAnAppVersionOnlyOnceFiveInstallationsReportIt(t *testing.T) {
	...
	reportInstalled(t, r, installs[4].AccessToken, "1.2.0", assetID)
...
func TestRevokingAnInstallationLeavesNoAppVersionOrNoticeBehind(t *testing.T) {
	...
	revoked := send(t, r, browserRequest(t, http.MethodDelete, "/v1/instances/"+revokedID, nil, session))
...
func TestAnInstallationThatLeavesOutItsAppVersionCountsUnderTheVersionItDeclared(t *testing.T) {
	...
	for _, install := range installs {
		...
		reportInstalled(t, r, install.AccessToken, "", assetID)
	}
	...
	reportInstalled(t, r, installs[0].AccessToken, "1.2.0", assetID)
	if versions := installedAppVersions(t, r, assetID); len(versions) != 0 {
...
func TestALibraryReportRefusesAnAppVersionThatIsNotShortPrintableText(t *testing.T) {
	...
	for _, bad := range []string{"1.2.0\a", strings.Repeat("9", 65)} {
		rec := send(t, r, asInstance(t, http.MethodPost, "/v1/library/sync", install.AccessToken, map[string]any{
			"snapshot":           false,
