...
type discordRepairSender interface {
	Request(context.Context, string, string, []byte) (outbound.Answer, error)
}

// RepairDiscord changes only the named announcement and its private audit record.
func (s *Service) RepairDiscord(ctx context.Context, actor, id uuid.UUID, in DiscordRepair) (DiscordRepairResult, error) {
	...
	if sendErr == nil {
		...
		case answer.Status >= 200 && answer.Status < 300:
			...
			} else if message := discord.MessageID(answer.Body); message != "" {
