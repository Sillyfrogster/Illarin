...
func archive(entries ...string) Find {
...
func TestImagesInTheArchiveMoveBesideTheirSection(t *testing.T) {
	...
	want := []Image{{Entry: "readme_img/ui global.png", Name: "Global settings"}, {Entry: "docs/list.png", Name: "Model list"}}
...
func TestTheFirstPictureBeforeAnySectionIsTheCover(t *testing.T) {
	...
	if page.Cover == nil || *page.Cover != (Image{Entry: "art/banner.png", Name: "Banner"}) {
		t.Fatalf("cover = %+v, want the banner", page.Cover)
	}
	if want := []Image{{Entry: "art/second.png", Name: "Second"}}; !reflect.DeepEqual(page.Opening.Images, want) {
...
func TestARemotePictureIsListedByItsAddress(t *testing.T) {
	...
	if got := page.Opening.Remote; len(got) != 1 || got[0] != (Remote{Address: "https://cdn.example.com/hero.jpg?raw=1", Name: "Hero"}) {
