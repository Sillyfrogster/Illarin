...
// Inbox lists an account's entries newest first, starting after the cursor when there is one.
func (s *Service) Inbox(ctx context.Context, account uuid.UUID, after *Cursor, limit int) (Page, error) {
	...
	rows, err := s.pool.Query(ctx, `
		select id, type, asset_id, words, created_at, read_at
		  from notifications
		 where account_id = $1
	...
	for rows.Next() {
		var entry Entry
		if err := rows.Scan(&entry.ID, &entry.Type, &entry.Asset, &entry.Words, &entry.CreatedAt, &entry.ReadAt); err != nil {
...
// Unread counts the entries the account has not opened.
