...
func TestNestingStopsBeforeItCanBeUsedToExhaustAReader(t *testing.T) {
	...
	_, err := postbody.Read([]byte(`{"version":1,"content":[` + body + `]}`))
	if err == nil {
...
func TestATableIsBoundedInBothDirections(t *testing.T) {
	...
	if _, err := postbody.Read([]byte(wide)); err == nil {
	...
	if _, err := postbody.Read([]byte(tall)); err == nil {
...
func TestAChosenHeadingAddressCannotTakeAWrittenOne(t *testing.T) {
	t.Parallel()
	_, err := postbody.Read([]byte(`{"version":2,"content":[
...
func TestEveryVocabularySetIsClosed(t *testing.T) {
	t.Parallel()
	for _, name := range postbody.Languages {
		body := fmt.Sprintf(
