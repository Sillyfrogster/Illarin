...
func TestEveryCarriedImportBecomesItsCanonicalDocument(t *testing.T) {
	...
	for name, one := range imports(t, "carried") {
		t.Run(name, func(t *testing.T) {
			document, warnings, err := postbody.FromMarkdown(one.Markdown)
			if err != nil {
...
func TestEveryRefusedImportNamesWhatStoppedIt(t *testing.T) {
	...
	for name, one := range imports(t, "refused") {
		t.Run(name, func(t *testing.T) {
			document, _, err := postbody.FromMarkdown(one.Markdown)
			if err == nil {
				t.Fatalf("%s was accepted as %s", one.Note, document)
			}
			refused, ok := err.(postbody.Refused)
			if !ok {
				t.Fatalf("refusal is %T, want a postbody.Refused", err)
			}
...
func TestAnImportKeepsNoMarkdownBesideTheDocument(t *testing.T) {
	...
	document, _, err := postbody.FromMarkdown(source)
	if err != nil {
...
func TestAnImportRefusesMoreMarkdownThanAPostCanHold(t *testing.T) {
	t.Parallel()
	_, _, err := postbody.FromMarkdown(strings.Repeat("word ", 200000))
	if err == nil {
		t.Fatal("an oversized import was accepted")
	}
}

func checkNotes(t *testing.T, said []postbody.Note, want []expectedNote) {
	t.Helper()
