...
func (s blogStack) fetch(t *testing.T, address string) *httptest.ResponseRecorder {
	t.Helper()
...
func TestAnAuthorPlacesAnUploadedPictureAndAReaderReceivesIt(t *testing.T) {
	...
	session := stack.siteAdmin(t, "pictures@example.com", "illarin.pictures")
	draft := stack.illarinDraft(t, session, "The workspace has pictures")

	picture := stack.uploaded(t, session, draft.ID, "body", apitest.PNG(t, 1200, 600))
	if picture.PostID != draft.ID || picture.Purpose != "body" {
	...
	written := stack.saved(t, session, draft.ID, finished(draft, map[string]any{
		"body": bodyWithPicture(picture.ID, "The workspace with one draft in it"),
	}))
...
func TestAPostCannotPlaceAPictureAnotherPostOwns(t *testing.T) {
	t.Parallel()
	stack := newBlogStack(t)
	session := stack.siteAdmin(t, "foreign@example.com", "illarin.foreign")
	...
	picture := stack.uploaded(t, session, theirs.ID, "body", apitest.PNG(t, 400, 400))
	refused := stack.save(t, session, mine.ID, finished(mine, map[string]any{
		"body": bodyWithPicture(picture.ID, "A picture from somewhere else"),
	}))
...
func TestAPictureIsPlacedOnlyWhereItWasUploadedFor(t *testing.T) {
	t.Parallel()
	stack := newBlogStack(t)
	session := stack.siteAdmin(t, "purpose@example.com", "illarin.purpose")
	...
	refused := stack.save(t, session, draft.ID, finished(draft, map[string]any{
		"body": bodyWithPicture(header.ID, "The header, placed in the body"),
	}))
	if refused.Code != http.StatusBadRequest {
...
func TestEveryDisplayedPictureCarriesAlternativeText(t *testing.T) {
	t.Parallel()
	stack := newBlogStack(t)
	session := stack.siteAdmin(t, "alt@example.com", "illarin.alt")
	...
	refused := stack.save(t, session, draft.ID, finished(draft, map[string]any{
		"body": bodyWithPicture(picture.ID, "   "),
	}))
	if refused.Code != http.StatusBadRequest {
		t.Fatalf("an undescribed picture status = %d, want 400: %s",
			refused.Code, refused.Body.String())
	}
	if !strings.Contains(refused.Body.String(), "body.content.1.alt") {
...
func TestReplacingAPictureLeavesTheOneAPublishedEditionCarries(t *testing.T) {
	t.Parallel()
	stack := newBlogStack(t)
	session := stack.siteAdmin(t, "replace@example.com", "illarin.replace")
	draft := stack.illarinDraft(t, session, "The picture that was replaced")

	first := stack.uploaded(t, session, draft.ID, "body", apitest.PNG(t, 900, 300))
	stack.saved(t, session, draft.ID, finished(draft, map[string]any{
		"body": bodyWithPicture(first.ID, "The first picture"),
	}))
	stack.published(t, session, draft.ID)
	published := stack.reader(t, draft.Slug).Media[0].URL

	second := stack.uploaded(t, session, draft.ID, "body", apitest.PNG(t, 600, 600))
	if second.ID == first.ID {
		t.Fatal("replacing a picture reused the address readers already hold")
	}
	written := stack.saved(t, session, draft.ID, finished(draft, map[string]any{
		"version": stack.reload(t, session, draft.ID).Version,
		"body":    bodyWithPicture(second.ID, "The second picture"),
	}))
...
func TestAnUploadRefusesBytesThatAreNotAPicture(t *testing.T) {
	t.Parallel()
	stack := newBlogStack(t)
	session := stack.siteAdmin(t, "notapicture@example.com", "illarin.notapicture")
	draft := stack.illarinDraft(t, session, "Only pictures go here")

	refused := stack.upload(t, session, draft.ID, "body", []byte("this is not a picture"))
	if refused.Code != http.StatusBadRequest {
...
func TestOnlyTheEditorOfAPostUploadsPicturesToIt(t *testing.T) {
	t.Parallel()
	stack := newBlogStack(t)
	session := stack.siteAdmin(t, "owner@example.com", "illarin.owner")
	outsider := stack.member(t, "outsider@example.com", "outsider.account")
	draft := stack.illarinDraft(t, session, "Not everyone writes here")

	refused := stack.upload(t, outsider, draft.ID, "body", apitest.PNG(t, 200, 200))
...
func (s blogStack) reload(t *testing.T, session *http.Cookie, id string) blogPost {
	t.Helper()
	response := apitest.Send(t, s.router, apitest.Authorized(httptest.NewRequest(
		http.MethodGet, "/v1/blog/posts/"+id, nil,
	), session))
