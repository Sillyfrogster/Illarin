...
func (s blogStack) imported(
	...
	return s.sent(t, who, jsonRequest(t,
		http.MethodPost, "/v1/blog/posts/"+id+"/import", string(body),
	))
}
...
func TestMarkdownAndJSONReachTheSameDocument(t *testing.T) {
	...
	body, err := json.Marshal(finished(written, map[string]any{
		"body": json.RawMessage(theSameThingAsJSON),
	}))
	if err != nil {
		t.Fatalf("encode the drafted changes: %v", err)
	}
	saved := stack.sent(t, writer, jsonRequest(t,
		http.MethodPut, "/v1/blog/posts/"+written.ID, string(body),
	))
	...
	if same(t, carried.Post.Body) != same(t, decodePost(t, saved).Body) {
		t.Errorf("Markdown became %s", same(t, carried.Post.Body))
	}
}
...
func TestAnImportSaysWhatItCouldNotCarryExactly(t *testing.T) {
	...
	if len(carried.Post.Body.Content) != 2 {
		t.Errorf("the post carries %d blocks", len(carried.Post.Body.Content))
	}
}
...
func TestAnImportRefusesEveryThingItCannotCarryAtOnce(t *testing.T) {
	...
	if refused.Code != string(blog.BlogErrorCodeInvalid) || refused.Field != "markdown" {
	...
	if left := stack.working(t, writer.session, draft.ID); len(left.Body.Content) != 0 {
		t.Errorf("a refused import still changed the drafted changes to %v", left.Body)
	}
}
...
func TestAnImportCannotPlaceAnotherPostsPicture(t *testing.T) {
	...
	picture := stack.uploaded(t, writer.session, theirs.ID, "body", apitest.PNG(t, 900, 500))
	...
	if refusalOf(t, response).Code != string(blog.BlogErrorCodeInvalid) {
...
func TestAnImportPlacesAPictureThePostOwns(t *testing.T) {
	...
	writer := stack.contributor(t, "writer@example.com", "publication.writer")
	draft := stack.draftBy(t, writer, "Lumiverse 3 is out")
	picture := stack.uploaded(t, writer.session, draft.ID, "body", apitest.PNG(t, 900, 500))

	carried := decodeImport(t, stack.imported(t, writer, draft.ID, fmt.Sprintf(
		"![A workspace](media:%s \"One draft.\")\n", picture.ID,
	)))
	placed := carried.Post.Body.Content
...
func TestAnImportBegunFromAnOlderVersionIsRefused(t *testing.T) {
	...
	response := stack.sent(t, writer, jsonRequest(t,
		http.MethodPost, "/v1/blog/posts/"+draft.ID+"/import", string(stale),
	))
	if response.Code != http.StatusConflict {
		t.Fatalf("import status = %d: %s", response.Code, response.Body.String())
	}
	if refused := refusalOf(t, response); refused.Code != string(blog.BlogErrorCodeStaleVersion) ||
		refused.Version == nil {
		t.Errorf("the refusal reads %+v", refused)
	}
}
...
func TestAnImportKeepsNoMarkdownAnywhere(t *testing.T) {
	...
	first := stack.sent(t, writer, jsonRequest(t,
		http.MethodPost, "/v1/blog/posts/"+draft.ID+"/import", string(body),
	))
	...
	for _, kept := range []struct {
		...
		{"the post", `select body::text from posts where id = $1`},
		{"a revision", `select coalesce(string_agg(body::text, ' '), '')
		                  from post_revisions where post_id = $1`},
		{"the action log", `select coalesce(string_agg(
		                        coalesce(before_state, '') || ' ' || coalesce(after_state, ''), ' '
		                    ), '') from blog_activity_log where post_id = $1`},
	} {
		var held string
...
func TestOnlyTheImportOperationTakesMarkdown(t *testing.T) {
	...
	writer := stack.contributor(t, "writer@example.com", "publication.writer")
	draft := stack.draftBy(t, writer, "Lumiverse 3 is out")
	body, err := json.Marshal(finished(draft, map[string]any{"body": theSameThing}))
	if err != nil {
		t.Fatalf("encode the drafted changes: %v", err)
	}
	response := stack.sent(t, writer, jsonRequest(t,
...
func (s blogStack) draftBy(t *testing.T, who contributor, title string) blogPost {
	t.Helper()
...
func same(t *testing.T, document postBody) string {
	t.Helper()
