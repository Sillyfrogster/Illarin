...
func (h *Handlers) UpdatePublicationCategory(c *gin.Context) {
	...
	updated, err := h.publications.UpdateCategory(
		c.Request.Context(), authority.ID, id, CategoryUpdate{
			Label:   request.Label,
...
func (h *Handlers) CreatePublicationGrant(c *gin.Context) {
	...
	made, err := h.publications.CreateGrant(c.Request.Context(), authority.ID, GrantEdit{
	...
	listed, err := h.withHolders(c, []Grant{made})
	if err != nil {
		return
	}
...
func (h *Handlers) UpdatePublicationGrant(c *gin.Context) {
	...
	change := GrantUpdate{DefaultCategoryID: (*uuid.UUID)(request.DefaultCategoryId)}
	...
	listed, err := h.withHolders(c, []Grant{updated})
	if err != nil {
		return
	}
...
func categoryOr(err error, otherwise PublicationErrorCode) PublicationErrorCode {
	if errors.Is(err, ErrCategoryRefused) {
...
func toAPIApps(configured []App) []PublicationApp {
...
func toAPIApp(found App) PublicationApp {
...
func toAPICategories(sorted []Category) []PublicationCategory {
...
func toAPICategory(found Category) PublicationCategory {
...
func toAPIGrant(found Grant, holder account.PublicProfile) PublicationGrant {
