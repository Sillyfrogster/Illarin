...
func sectionsNamed(count int) []readme.Section {
	sections := make([]readme.Section, 0, count)
	for index := range count {
		sections = append(sections, readme.Section{
...
func TestAWaitingPictureJoinsItsBlockBesideTheWriting(t *testing.T) {
	...
	media := uuid.New()
	after, _, err := placePicture(page, VaultPicture{MediaID: &media, Name: "Settings", BlockID: &page[0].ID})
	if err != nil {
	...
	second := uuid.New()
	again, _, err := placePicture(after, VaultPicture{MediaID: &second, Name: "More", BlockID: &page[0].ID})
	if err != nil {
