...
func TestRevocationRejectsAnUploadWaitingForCandidateAcceptance(t *testing.T) {
	...
	if err := <-done; !errors.Is(err, ErrAssetFrozen) {
		t.Fatalf("revoked acceptance = %v, want frozen", err)
	}
	if err := svc.ClearWithhold(context.Background(), id); err != nil {
		t.Fatal(err)
	}
	_, err = svc.AcceptRevision(context.Background(), RevisionInput{
...
func TestQueuedRevisionCannotOverwriteANewerWorkingCopy(t *testing.T) {
	...
	adult := false
	if err := svc.SetIdentity(context.Background(), Identity{OwnerID: owner, AssetID: created.ID, Name: "Newer work", IsNSFW: &adult}, candidate); err != nil {
		t.Fatal(err)
	}
	...
	page, err := svc.WorkingCopy(context.Background(), created.ID, &owner, ContentShown)
	if err != nil {
		t.Fatal(err)
	}
