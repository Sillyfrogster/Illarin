...
import { importCounts, positionAt } from "./shelf-places";
...
export const DRAG_QUERY = "(min-width: 1024px) and (pointer: fine)";
...
export function ShelfProvider({ children }: { children: ReactNode }) {
  ...
  function follow(event: DragMoveEvent) {
  ...
  function drop(event: DragMoveEvent) {
    ...
    if (from && to) setFlight({ from, to, title: sectionName(piece) });
