...
export function toEditor(body: PostBody, media: PostMedia[]): JSONContent {
  const held = new Map(media.map((one) => [one.id, one]));
  return {
    type: "doc",
    content: body.content.map((block) => editorBlock(block, held)),
  };
}

export function fromEditor(content: JSONContent): PostBody {
  return {
