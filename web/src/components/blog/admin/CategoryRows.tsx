...
export function CategoryRows({
  ...
  onOpen: (category: BlogCategory) => void;
  onOrdered: (categories: BlogCategory[]) => void;
  onSaved: (saved: BlogCategory) => void;
}) {
  const current = categories.filter((one) => !one.retired);
  const retired = categories.filter((one) => one.retired);

  const [placing, setPlacing] = useState<BlogCategory[] | null>(null);
  ...
  async function bringBack(category: BlogCategory) {
