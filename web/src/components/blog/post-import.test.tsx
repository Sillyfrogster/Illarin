...
import { POST_BODY_VERSION, type PostBody } from "@/lib/post-body";
...
type ImportCase = { note: string; markdown: string; body: PostBody };
...
for (const [name, one] of CARRIED) {
  test(`${name} is written at the version this build emits`, () => {
    expect(one.body.version).toBe(POST_BODY_VERSION);
  });

  test(`${name} survives a round trip through the editor`, () => {
    expect(fromEditor(toEditor(one.body, HELD))).toEqual(one.body);
  });
