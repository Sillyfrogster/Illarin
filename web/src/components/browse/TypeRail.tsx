"use client";

import Link from "next/link";
import { useState } from "react";
import { TravellingHighlight } from "@/components/ui/travelling-highlight";
import type { BrowseFilters, BrowseType } from "@/lib/api/query";
import { buildBrowseHref } from "@/lib/browse-url";
import { cn } from "@/lib/cn";
import { TypeMark } from "./TypeMark";

const TYPES: { key: string; type?: BrowseType; label: string }[] = [
  { key: "all", label: "Everything" },
  { key: "character", type: "character", label: "Characters" },
  { key: "lorebook", type: "lorebook", label: "Lorebooks" },
  { key: "preset", type: "preset", label: "Presets" },
  { key: "theme", type: "theme", label: "Themes" },
  { key: "pack", type: "pack", label: "Packs" },
  { key: "extension", type: "extension", label: "Extensions" },
];

export function TypeRail({
  basePath,
  filters,
  navigate,
}: {
  basePath: string;
  filters: BrowseFilters;
  navigate: (next: BrowseFilters) => void;
}) {
  const chosen = filters.type ?? "all";
  const [lit, setLit] = useState(chosen);

  return (
    <nav aria-label="Type" className="min-w-0">
      <div className="overflow-x-auto p-1 [scrollbar-width:none] max-md:[mask-image:linear-gradient(to_right,#000_calc(100%-3rem),transparent)] [&::-webkit-scrollbar]:hidden">
        <TravellingHighlight
          chosen={chosen}
          className="flex w-max gap-1 max-md:pr-10"
          onLit={setLit}
        >
          {TYPES.map((one) => (
            <Link
              aria-current={chosen === one.key ? "page" : undefined}
              className={cn(
                "flex min-h-11 items-center gap-2 rounded-control px-4 font-ui text-ui font-medium whitespace-nowrap outline-offset-2 transition-colors duration-200 motion-reduce:transition-none",
                lit === one.key ? "text-on-accent" : "text-mute",
              )}
              data-cell={one.key}
              href={buildBrowseHref(
                { ...filters, type: one.type, facet: undefined },
                basePath,
              )}
              key={one.key}
              onClick={(event) => {
                event.preventDefault();
                navigate({ ...filters, type: one.type, facet: undefined });
              }}
            >
              {one.type ? (
                <TypeMark className="size-4" type={one.type} />
              ) : null}
              {one.label}
            </Link>
          ))}
        </TravellingHighlight>
      </div>
    </nav>
  );
}
