...
import {
  fromEditor,
  toEditor,
} from "@/components/blog/writing/tiptap-document";
import {
  isPostAnchor,
  isWritten,
  POST_BODY_VERSION,
...
for (const [name, one] of corpus("invalid")) {
  test(`${name} loses what Go refuses before it reaches the drafted changes`, () => {
    ...
    expect(carried.version).toBe(POST_BODY_VERSION);
...
test("the editor draws a picture from the address Go answered with", () => {
  ...
  const editing = toEditor(body, HELD);
  ...
  expect(fromEditor(editing)).toEqual(body);
});
...
test("a gallery keeps the order the author put its pictures in", () => {
  ...
  expect(fromEditor(toEditor(body, HELD))).toEqual(body);
});
