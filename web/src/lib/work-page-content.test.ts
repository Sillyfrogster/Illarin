...
describe("reader page content", () => {
  test("Character, Voice and Scenario stay in the character block", () => {
    ...
    const result = splitWorkPageContent([character]);
  ...
  test("only system and post-history instructions move to the disclosure", () => {
    ...
    const result = splitWorkPageContent([instructions]);
  ...
  test("ordinary public content remains in its block and empty content does not render", () => {
    ...
    const result = splitWorkPageContent([changelog]);
  ...
  test("ordinary and disclosed elements split without emptying their block", () => {
    ...
    const result = splitWorkPageContent([mixed]);
...
describe("a work that holds nothing", () => {
  ...
  test("every block empty means the work holds nothing", () => {
    expect(workHoldsNothing([core("The character"), core("Messages")])).toBe(
      true,
    );
  });
  ...
  test("a work with no blocks at all holds nothing", () => {
    expect(workHoldsNothing([])).toBe(true);
  });
