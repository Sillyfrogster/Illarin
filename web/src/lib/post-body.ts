export const POST_BODY_VERSION = 2;
...
export type PostBody = { version: number; content: PostBlock[] };

export function emptyPostBody(): PostBody {
  return { version: POST_BODY_VERSION, content: [] };
}

export function isWritten(body: PostBody): boolean {
  return textLength(body.content) > 0;
}
...
export function asPostBody(value: {
  version: number;
  content: Record<string, unknown>[];
}): PostBody {
  return value as unknown as PostBody;
}
